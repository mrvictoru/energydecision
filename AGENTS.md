...
- **Household**: ausgrid solar-battery (SolarBatteryEnv) — on the **legacy Ausgrid benchmark** the DT beats Oracle; on the **modern 2019+ real-telemetry rebuild** the perfect-foresight oracle instead dominates (best DT gross +$293–357/yr vs RTE-matched oracle +$690/yr). Only the **unconstrained 7 d real-OOD DT is net-positive** (+$83–93/yr); constrained/long-horizon surfaces are net-negative (30 d −$114, 90 d −$238 unconstrained; directional/price-gate 30–90 d −$45 to −$108) and clip at the SOC rails (net-of-wear A9). See `docs/household/workflow.md` §H4.9 bucket-A table and `docs/known_issues.md` A6/A9.
- **AEMO / grid-scale**: NEM market + FCAS (AEMOBatteryTradingEnv) — the **standalone Decision Transformer is the preferred policy**. The **physics-v2** Stage C DT (`aemo_dt_sdp_jtsoc_v2cal.pt`, corrected degradation/efficiency physics + wear-calibrated SDP-teacher trajectories + J_t(soc) RTG, `rtg_mode="auto"`) beats PPO on **all 4 extended identity surfaces** (standard-2024 3.84× n=30, dispatch-matched 1.80× n=12, expanded broad-2024 6.70× n=30, 2025 OOD 6.39× n=6; all four paired CIs exclude zero) **and passes the market-impact gate** (9/9 cells; fixed `rtg0.0` fallback small 4.36× / hornsdale 4.71× / torrens 4.67× vs PPO). It supersedes the v1 Stage C (`aemo_dt_sdp_jtsoc_fullcorpus.pt`, standard 4.9× / dispatch 1.57× / expanded 1.78× / 2025 3.98×) and the earlier small-n single-month surfaces (canonical Standard Oct n=5 is unaffected; canonical dispatch/expanded baselines were stale — see `paper/audit/artifacts/cache_data_staleness_finding.md`). The earlier "explicit j_t_soc collapses under impact" finding was a `return_scale` evaluation bug (see `docs/known_issues.md` B8). **Household caveat:** the household re-baseline is *complete for buckets A+B* — calibrated `h4_v2c` teacher, RTE-matched oracle, retrained forecast arms (the pre-fix TTM advantage did not survive) and 9 retrained SB3 checkpoints; only bucket C (live shadow, spot pass-through, broader telemetry) is open. Provenance audit of the shipped AEMO numbers: `paper/audit/audit_findings.md` + `paper/audit/provenance.tsv` (2026-09-21; regenerates the v2cal significance and impact-gate artifacts).
...
| **Batch size** | 64 is the default for the proxy/learning tiers. 16 works for 8×512 expanded eval on this hardware. |
...
- **FCAS-rich dataset**: `data/aemo_dt_fcas/aemo_fcas_dataset.parquet` (2,425 episodes, 78.4M rows, 3.1 GB)
  ...
  - Historical — used by the Jul 2026 v2 pretrained model; the Stage C DT uses SDP-teacher trajectories
- **SDP-teacher trajectories** (`data/aemo_dt_sdp/`): the training data for the shipped Stage C DT
  - `dt_trajectories_full.parquet` (320 eps, conservative deg=$50/MWh), `dt_trajectories_aggressive.parquet` (320 eps, deg=$20/MWh)
  - `dt_trajectories_jtsoc_combined.parquet` (640 eps, J_t(soc) RTG — used by shipped model)
  - Generated by `scripts/generate_sdp_dt_trajectories.py` using the honest SDP executor (`src/aemo_sdp_executor.py`)
...
> **Note:** The current SOTA is the **physics-v2 Stage C standalone DT** (`models/aemo/dt/aemo_dt_sdp_jtsoc_v2cal.pt`, `--rtg-source j_t_soc`, `--auto-return-scale`, wear-calibrated teacher), which beats PPO on all 4 identity surfaces + the impact gate (see `report.md §8.2.11`). It supersedes v1 `aemo_dt_sdp_jtsoc_fullcorpus.pt`. **Weights and the training corpus are hosted on Hugging Face**: model [`mrvictoru/energydecision-dt-v2-sdp`](https://huggingface.co/mrvictoru/energydecision-dt-v2-sdp) (`aemo_dt_sdp_jtsoc_v2cal.pt` + `.meta.json` sidecar; config via the sidecar, not an embedded key), dataset [`mrvictoru/AEMO_simulated_trade_sdp`](https://huggingface.co/datasets/mrvictoru/AEMO_simulated_trade_sdp) (`dt_trajectories_jtsoc_v2cal_conservative.parquet`). The HF checkpoints below and GRPO post-training are historical.
