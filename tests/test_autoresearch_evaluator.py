...
def test_summarize_loss_history_reads_final_and_best(tmp_path: Path):
    ...
    with loss_csv.open('w', newline='', encoding='utf-8') as fh:
        ...
        writer.writerow([1, 1.2, 1.0, 0.1, 0.1, 0.8, 0.5, 0.2, 0.1])
        writer.writerow([2, 1.0, 0.8, 0.1, 0.1, 0.6, 0.6, 0.1, 0.2])
        writer.writerow([3, 0.9, 0.7, 0.1, 0.1, 0.7, 0.4, 0.3, 0.05])
    ...
    assert summary['final_val_action_loss'] == pytest.approx(0.4)
    assert summary['best_val_epoch'] == 2
    assert summary['best_val_total_loss'] == pytest.approx(0.6)
    assert summary['best_val_action_epoch'] == 3
    assert summary['best_val_action_loss'] == pytest.approx(0.4)
    assert summary['best_val_state_epoch'] == 2
    assert summary['best_val_return_epoch'] == 3
...
def test_evaluate_aemo_heldout_reuses_cached_reference_rollouts(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    def fake_run_policy_episodes(**kwargs):
        ...
        if policy_name == 'candidate_dt':
            return [_episode([3.0, 2.0], [{'battery_soc': 5.0, 'capacity_mwh': 10.0}, {'battery_soc': 4.0, 'capacity_mwh': 10.0}])]
    ...
    first = evaluator.evaluate_aemo_heldout(
        surface_manifest={'paths': {}},
        training_summary={'best_val_total_loss': 0.4},
        evaluation_config=evaluation_config,
    ...
    second = evaluator.evaluate_aemo_heldout(
        surface_manifest={'paths': {}},
        training_summary={'best_val_total_loss': 0.3},
        evaluation_config=evaluation_config,
