...
def test_main_serializes_datetime_entries_in_manifest(tmp_path: Path, monkeypatch):
    config_path = tmp_path / "eval.json"
    config_path.write_text(
        json.dumps(
            {
    ...
    monkeypatch.setattr(
        ...
        "fetch_and_preprocess_aemo_scenarios",
        lambda **kwargs: (
            {"heldout_sa1": pl.DataFrame({"RRP": [1.0]})},
            [
                {
                    "label": "heldout_sa1",
                    "region": "SA1",
    ...
    manifest = json.loads(output_path.read_text(encoding="utf-8"))
    assert manifest["cache_preflight"][0]["start_date"] == "2024-07-01 00:00:00"
    assert manifest["scenario_manifest"][0]["start_date"] == "2024-07-01 00:00:00"
