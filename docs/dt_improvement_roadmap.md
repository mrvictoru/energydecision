...
PPO learned FCAS bidding strategies through millions of online interactions with the AEMO environment. DT was trained on offline episodes — none of which came from policies that aggressively participated in FCAS markets.

**Current training dataset** (2,425 episodes, 78.4M rows):
- PPO (905 eps): strong FCAS signal ($8.98/step)
- FCAS rule (300 eps): moderate FCAS signal ($2,941/ep)
- A2C/DDPG/SAC/TD3 (300 eps each): moderate FCAS signal
- Old rule (20 eps): no FCAS signal

The FCAS gap should now be addressable — the training data finally contains FCAS-active examples.
...
| Criterion | Target | Current (old DT) |
|-----------|--------|-------------------|
| Mean reward | > 0.0 | -3.11 |
| FCAS revenue | > $2,000/ep | $77/ep |
| Profit/ep | > $2,000/ep | -$1,396 |
| Beat FCAS rule baseline | yes | not tested |

---

## 4. Open Questions

1. **Can DT learn FCAS bidding from PPO demonstrations, or does FCAS require online interaction?**
2. **Is the DT architecture sufficient for multi-market strategies?**
3. **How much FCAS data is "enough"?** The learning curve from 300→905 PPO eps.
4. **Does RTG-conditioning work as a strategy selector for AEMO?**
5. **Is the gap mainly data quality, data quantity, or architecture?**
