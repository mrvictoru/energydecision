# Documentation

This is the main human documentation entrypoint for the repository.

## Start Here

- [../README.md](../README.md): project overview and quick navigation
- [architecture.md](architecture.md): how the repository is organized
- [development.md](development.md): setup, runtime modes, tests, and contributor workflow

## By Goal

### Learn the codebase

- [architecture.md](architecture.md)
- [../COMPONENTS.md](../COMPONENTS.md)
- [AGENT_README.md](AGENT_README.md)
- [HELPER_README.md](HELPER_README.md)

### Reproduce the household track

- [household/README.md](household/README.md)
- [household/workflow.md](household/workflow.md)
- [household/environment.md](household/environment.md)
- [household/degradation.md](household/degradation.md)

### Reproduce the AEMO track

- [aemo/README.md](aemo/README.md)
- [aemo/workflow.md](aemo/workflow.md)
- [aemo/environment.md](aemo/environment.md)
- [evaluation_guide.md](evaluation_guide.md)

### Develop on the repo

- [development.md](development.md)
- [DP_ALGORITHM_README.md](DP_ALGORITHM_README.md)
- [AGENT_README.md](AGENT_README.md)

## Stable Reference Docs

- [aemo/environment.md](aemo/environment.md)
- [aemo/degradation.md](aemo/degradation.md)
- [aemo/dispatch-replay.md](aemo/dispatch-replay.md)
- [household/environment.md](household/environment.md)
- [household/degradation.md](household/degradation.md)
- [evaluation_guide.md](evaluation_guide.md)

## Research Notes And Status Docs

These files are useful, but they are not the best place to start if you are new to the repo.

- [research/README.md](research/README.md)
- [grpo_experiments.md](grpo_experiments.md)
- [aemo_research_plan.md](aemo_research_plan.md)
- [modern_transformer_improvements.md](modern_transformer_improvements.md)

## Legacy Deep References

These documents still contain important detail, but they are deeper and less curated than the main entrypoints above.

- [../COMPONENTS.md](../COMPONENTS.md)
- [AGENT_README.md](AGENT_README.md)
- [HELPER_README.md](HELPER_README.md)
- [DP_ALGORITHM_README.md](DP_ALGORITHM_README.md)