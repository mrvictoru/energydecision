...
| ✅ **H2** | **SDP-teacher distillation transfer test completed**: checkpoint-specific configs now persist alongside weights; 840/180 synthetic teacher episodes were regenerated under 31.042c import, free 11:00–14:00, 1c FiT; inference precomputes the exact segment-local `-J_t(soc)` prompt per calendar day. The 2×128 DT achieved best validation total/action loss 0.0609/0.0608, but on five real OOD segments it **loses $53/year** versus no battery (95% bootstrap bill CI $876–$2,412), below PPO (+$2), legacy DT (+$3), rule (+$82), and oracle (+$894). | **Negative transfer confirmed**: correcting configuration, tariff, and inference prompting does not overcome the synthetic-corpus ceiling; planner distillation has not transferred across scales |
