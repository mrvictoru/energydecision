...
| 🟡 **H2** | **Ported SDP-teacher distillation**: 840 synthetic train + 180 validation episodes generated (1,693,440 + 362,880 rows), then a 2×128 standalone DT trained for 5 epochs with `rtg_value = -J_t(soc)` (best validation total/action loss 0.0424/0.0423). On the five-segment real OOD surface it saves $27/year, better than the cloning-era DT (−$6) but below rule ($70) and oracle ($729). | Negative transfer result. OPEN DEFECTS: (1) config persistence — the saved `h2_sdp_jtsoc_best.pt` has `max_timestep=2016` (7-day episode length) but the shared `decision_transformer_model_kwargs.json` still carries the cloning-era `17567`, so `evaluate_household_ood_baselines.py` crashes on load (verified checkpoint is valid under corrected config); (2) inference uses the standard realized-reward RTG recurrence, NOT per-step `J_t(soc)` recomputation, so the distillation signal is unused at decision time; (3) teacher data + H1 baselines still on legacy 30c/5c flat tariff, not the realistic 31.042c/free-11-14h/1c-FiT used by H3. Fix all three, then re-evaluate. |
