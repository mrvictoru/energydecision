...
| ✅ **H4.5** | **Degradation-aware policy study** — compare degradation disabled, cycle-only, calendar-plus-cycle, and realistic battery-life-cost settings across the horizon-diverse corpus. | **Done (2026-09-06).** Final definitive run: 5 conditions × 3 seeds on the fixed 10-window real-OOD surface, using the 24-hour persistence forecast channels (not the TTM sidecar — comparable to H4.4 only in surface, not forecast input), with the evaluator now reporting both **grid-bill-only savings** and **net-of-wear savings** using each policy’s actual `info["step_degradation"]` and the default battery-life cost of **A$5,000**. Primary net-of-wear economics (A$/yr vs no battery): degradation disabled **−A$341.1**, cycle-only **−A$531.3**, full realistic **−A$438.4**, high cost (**A$10,000**) **−A$468.5**, low cost (**A$1,000**) **−A$558.7**. Secondary grid-bill-only means were: degradation disabled **+A$367.1**, cycle-only **+A$292.8**, full realistic **+A$359.8**, high cost **+A$326.8**, low cost **+A$322.4**. The key finding is that the no-degradation policy looks best on energy-only savings but loses most of its advantage once wear is priced in; this supports the hypothesis that degradation-aware control is economically meaningful. The A$1,000/A$10,000 inversion does **not** survive across the three-seed aggregate: it collapses and no stable dose-response ordering remains. Pairwise checks across windows show the grid-bill ranking is significant (e.g., disabled vs cycle-only mean diff **+A$74.4/yr**, 95% CI **A$41.6–A$106.9**, p=0.00195), while net-of-wear differences are economically meaningful but noisier (e.g., disabled vs cycle-only mean diff **+A$190.2/yr**, 95% CI **−A$33.5–A$398.4**, p=0.0967). The final result supersedes the earlier single-seed pilot. Artifacts: `results/h4_5_degradation/` and `eval_output/household/h4_5_degradation/`. |

> **Diary (2026-09-06):** We completed the definitive H4.5 multi-seed rerun. The earlier pilot was useful as a sanity check, but it was not the source of truth because it measured only grid-bill reduction and used a single seed. The final scorecard treats the net-of-wear metric as primary and the grid-bill-only metric as secondary. The key operational takeaway is that “aggressive cycling” is not economically free once degradation is priced in; the no-degradation policy is penalized most heavily, and the $1k/$10k cost sweep does not show a robust monotone effect across seeds.
