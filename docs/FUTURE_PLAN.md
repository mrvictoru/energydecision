...
| Price-aware gating | 🟡 Promising | A conservative gate (`min discharge $0.30/kWh`, `max grid charge $0.10/kWh`) improved the same three 90-day real windows from approximately +A$1.1/year to +A$3.7/year, with unchanged 0.049974 EFC/day and zero SOC clips/safety penalty. | Run a threshold sweep and broader seasonal coverage before selecting a deployment default. |
