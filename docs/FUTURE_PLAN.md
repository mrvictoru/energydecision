...
|  |  |  |
| **H4.9** | **Long-horizon evaluation** — 30-day / 90-day / 1-year evaluation horizons with realistic seasonal price/solar profiles. 7-day windows are too short for degradation economics to converge. | **Pilot complete:** 30-day real-normalized OOD pilot across DT, PPO, SAC, TD3, rule, oracle, and no-battery on 4 contiguous windows with realistic tariff, persistence forecast, and 12-worker reference parallelism (`eval_output/household/h4_9_pilot_30d/summary.json`). **Remaining:** expand to 90-day/1-year seasonal coverage and add explicit safety-clipping aggregates. |
