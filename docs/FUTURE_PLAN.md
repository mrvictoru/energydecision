...
| 🟡 **H4.5** | **Degradation-aware policy study** — compare degradation disabled, cycle-only, calendar-plus-cycle, and realistic battery-life-cost settings across the horizon-diverse corpus. | **Implemented** (scripts/h4_degradation_study.py ready) — **study execution pending** (separate session). Report whether the DT learns economically meaningful cycling restraint and whether degradation changes the policy ranking. |
