...
| 🟡 **H4.3** | **Fresh modern-data SB3 baseline** — fresh PPO was trained for 250k steps on the H4.1 train split with 12 parallel CPU environments and evaluated at the matched 5 kWh/3.3 kW configuration. On the five real OOD segments it saves **+$27/yr**, below the rule **+$81/yr**; the legacy PPO remains only a historical transfer baseline. | Initial PPO baseline complete but does not yet meet the improvement target. Repeat with longer/multi-seed training and optionally SAC/TD3; retain identical real OOD and synthetic held-out surfaces with bootstrap CIs and matched tariff/degradation settings. |
