...
| Multi-seed RL baselines | 🟡 Synthetic long-horizon evaluation complete; fair deployment comparison pending | Three seeds each at 250k timesteps completed for PPO, SAC, and TD3 on the H4.1 training corpus. On 20 shared held-out 180-day synthetic windows with raw policies, PPO achieved −A$12.22 to −A$54.40/year, SAC −A$30.18 to −A$44.55/year, and TD3 −A$0.11 to −A$661.79/year. TD3 also produced 0.75–1.03 million SOC clips and A$0.258–0.259M safety penalties per low-activity seed, while the collapsed seed reached 0.58 EFC/day and 6.69% mean capacity fade; PPO/SAC had materially fewer interventions. | Apply the same SOC/throughput/price wrappers to DT and all nine RL checkpoints, then validate on real OOD windows before comparative claims. |
...
|  |  |  |  |
| **H4.7** | **Production hardening** — close the sim-to-real gap for household deployment. | **Progress:** (1) ✅ Hard SOC limits implemented in `EnergySimEnv.step()` with validated fractional bounds, warning-based clipping, enforcement opt-out, `SafetyViolation`, and A$0.25 reward shaping; 12 focused environment tests and the full 360-test suite pass. (2) ✅ offline 14-day shadow proxy; (3) ✅ multi-seed PPO/SAC/TD3 training and shared 180-day synthetic deployment evaluation. The raw-policy evaluation exposed substantial seed sensitivity and TD3 safety-clipping failures. **Remaining:** fair wrapper-applied DT/RL comparison, broader real-OOD validation, and live shadow mode (unavailable with the current hardware interface). | Hard constraints: no retraining for the fair inference comparison; live shadow remains a prerequisite before closed-loop trial. |
|  |  |  |
| **H4.8** | **Cycle-throughput / capacity-fade telemetry** — already implemented in evaluator (`_cycle_metrics_from_logs`, `segment_efc`, `segment_cycles`, `segment_capacity_fade`, `mean_efc_per_day`, etc.). | Use the new per-window cycle metrics to strengthen H4.5 claims with direct mechanism evidence. | ✅ Implemented (eval script + `h4_degradation_study.py` aggregation). |
