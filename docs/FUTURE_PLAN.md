...
| 🟡 **H4.2** | **Forecast ablation and improved forecast generation** — inference-only ablation completed on ten deterministic seven-day real-OOD windows: persistence **+$92.28/yr**, zeroed **+$92.88/yr**, shuffled **+$95.78/yr**. The current 24-hour-persistence channels provide no measurable benefit to the H2 DT. The evaluator now supports cadence-aware bounded windows, exact provenance, and `persistence`/`zero`/`shuffle` modes. | Initial ablation complete. Train causal rolling/seasonal forecast variants and retrain matched policies before claiming forecast value; inference-only substitution cannot test a forecast distribution the model never learned. |
