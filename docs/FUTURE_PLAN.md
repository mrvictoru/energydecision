...
| Multi-seed RL baselines | 🟡 Fair synthetic deployment comparison complete; real-OOD comparison pending | Three seeds each at 250k timesteps completed for PPO, SAC, and TD3 on the H4.1 training corpus. A broader matched 5 kWh / 3.3 kW comparison applies identical 1%–99% SOC, independent 0.05 charge/discharge EFC/day, and `$0.30/$0.10` price-gate wrappers across ten shared 90-day windows spanning five archetypes and two horizons. DT (`rtg=-4`) achieved +A$3.55/year with 0.04997 EFC/day, 0 SOC clips, and 0 safety penalty; PPO/SAC were mixed across seeds and TD3 was mostly under-active. | Validate the shared-wrapper comparison on real OOD windows; live shadow mode is unavailable with the current hardware interface. |
...
|  |  |  |  |
| **H4.7** | **Production hardening** — close the sim-to-real gap for household deployment. | **Progress:** (1) ✅ Hard SOC limits implemented in `EnergySimEnv.step()` with validated fractional bounds, warning-based clipping, enforcement opt-out, `SafetyViolation`, and A$0.25 reward shaping; 12 focused environment tests and the full 360-test suite pass. (2) ✅ offline 14-day shadow proxy; (3) ✅ multi-seed PPO/SAC/TD3 training; (4) ✅ broader shared-wrapper DT/RL evaluation on ten 90-day synthetic windows. The fair result shows DT at +A$3.55/year with zero SOC clips and zero safety penalty, while RL remains seed-sensitive. **Remaining:** fair comparison on real OOD windows; live shadow mode is unavailable with the current hardware interface. | Hard constraints: no retraining for the fair inference comparison. |
|  |  |  |
| **H4.8** | **Cycle-throughput / capacity-fade telemetry** — already implemented in evaluator (`_cycle_metrics_from_logs`, `segment_efc`, `segment_cycles`, `segment_capacity_fade`, `mean_efc_per_day`, etc.). | Use the new per-window cycle metrics to strengthen H4.5 claims with direct mechanism evidence. | ✅ Implemented (eval script + `h4_degradation_study.py` aggregation). |
