...
| 🟡 **H2** | **Port the AEMO playbook**: `scripts/generate_household_sdp_trajectories.py` now rolls deterministic SDP-teacher actions through synthetic `SolarBatteryEnv` episodes and emits the shared `TrajectoryDataset` schema with exact per-step `rtg_value = -J_t(soc)`. | Full 840-episode teacher corpus, standalone DT training, and real-OOD tier evaluation remain |
