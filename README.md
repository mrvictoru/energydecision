...
*   [x] **AEMO preferred-policy recommendation (physics-v2, 2026-09):** The standalone DT ships with **surface-aware `rtg_mode="auto"`** on corrected degradation/efficiency physics and a **wear-calibrated SDP teacher** (`aemo_dt_sdp_jtsoc_v2cal.pt`). Identity surfaces: standard **$16.2k/ep vs PPO $2.35k** (6.9×), dispatch-matched **$40.0k vs $22.5k** (1.78×), expanded broad-2024 **$32.1k vs $19.5k** (1.65×), 2025 OOD **$30.8k vs $6.5k** (4.74×); passes the market-impact gate. Supersedes v1 (`aemo_dt_sdp_jtsoc_fullcorpus.pt`). Weights + corpus on Hugging Face: [mrvictoru/energydecision-dt-v2-sdp](https://huggingface.co/mrvictoru/energydecision-dt-v2-sdp) / [AEMO_simulated_trade_sdp](https://huggingface.co/datasets/mrvictoru/AEMO_simulated_trade_sdp). See `report.md §8.2.10/§8.2.11` and `docs/known_issues.md`. Household H4.x remain historical pending its own wear calibration (known_issues A6).
