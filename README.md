...
*   [x] **AEMO preferred-policy recommendation (physics-v2, 2026-09):** The standalone DT ships with **surface-aware `rtg_mode="auto"`** on corrected degradation/efficiency physics and a **wear-calibrated SDP teacher** (`aemo_dt_sdp_jtsoc_v2cal.pt`). Identity surfaces (extended, fresh): standard-2024 **$16.5k/ep vs PPO $4.3k** (3.84×, n=30), dispatch-matched **$33.8k vs $18.8k** (1.80×, n=12), expanded broad-2024 **$29.1k vs $4.4k** (6.70×, n=30), 2025 OOD **$30.8k vs $4.8k** (6.39×, n=6) — all four paired-difference 95% CIs exclude zero; passes the market-impact gate (9/9 cells; fixed `rtg0.0` fallback small 4.36× / hornsdale 4.71× / torrens 4.67× vs PPO). Supersedes v1 (`aemo_dt_sdp_jtsoc_fullcorpus.pt`). Weights + corpus on Hugging Face: [mrvictoru/energydecision-dt-v2-sdp](https://huggingface.co/mrvictoru/energydecision-dt-v2-sdp) / [AEMO_simulated_trade_sdp](https://huggingface.co/datasets/mrvictoru/AEMO_simulated_trade_sdp). See `report.md §8.2.10/§8.2.11` and `docs/known_issues.md`. **Household re-baseline complete (buckets A+B, 2026-09-20):** calibrated `h4_v2c` teacher, RTE-matched oracle, retrained forecast arms (the pre-fix TTM advantage did not survive) and 9 retrained SB3 checkpoints; only bucket C (live shadow, spot pass-through, broader telemetry) remains open (`docs/household/workflow.md` §H4.9).
...
- Hugging Face models:
  - [Stage C Decision Transformer (shipped, SDP-teacher distilled)](https://huggingface.co/mrvictoru/energydecision-dt-v2-sdp) — `aemo_dt_sdp_jtsoc_v2cal.pt` (physics-v2; supersedes `aemo_dt_sdp_jtsoc_fullcorpus.pt`); also mirrored in-repo at `models/aemo/dt/`
