...
*   [x] **AEMO preferred-policy recommendation (physics-v2, 2026-09):** The standalone DT ships with **surface-aware `rtg_mode="auto"`** on corrected degradation/efficiency physics and a **wear-calibrated SDP teacher** (`aemo_dt_sdp_jtsoc_v2cal.pt`). Identity surfaces: standard **$16.2k/ep vs PPO $2.35k** (6.9×), dispatch-matched **$40.0k vs $22.5k** (1.78×), expanded broad-2024 **$32.1k vs $19.5k** (1.65×), 2025 OOD **$30.8k vs $6.5k** (4.74×); passes the market-impact gate. Supersedes v1 (`aemo_dt_sdp_jtsoc_fullcorpus.pt`). Weights + corpus on Hugging Face: [mrvictoru/energydecision-dt-v2-sdp](https://huggingface.co/mrvictoru/energydecision-dt-v2-sdp) / [AEMO_simulated_trade_sdp](https://huggingface.co/datasets/mrvictoru/AEMO_simulated_trade_sdp). See `report.md §8.2.10/§8.2.11` and `docs/known_issues.md`. **Household re-baseline complete (buckets A+B, 2026-09-20):** calibrated `h4_v2c` teacher, RTE-matched oracle, retrained forecast arms (the pre-fix TTM advantage did not survive) and 9 retrained SB3 checkpoints; only bucket C (live shadow, spot pass-through, broader telemetry) remains open (`docs/household/workflow.md` §H4.9).
