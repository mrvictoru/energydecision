...
*   [ ] **Regime-shift / full-scale robustness evaluation:** Train on 2022–2023 and evaluate on 2024 (and the reverse), plus the expanded surface (5 regions × 6 months) with paired significance tests on the SOTA headline (modern v2 vs PPO vs forecast DT). **Measurement done 2026-08-07 (expanded sweep, now on the correct 5-min protocol — the previous 30-min protocol nearly halved the DT): modern v2 DT $4.6k/ep vs PPO $15.0k/ep (~3.3×), RTG-insensitive — the DT's SOTA is surface-specific (wins Jan/Oct, loses FCAS-spike months); paired significance still open.** Phase 6 showed FCAS tails are non-stationary across periods, so this tests whether the DT generalizes to genuinely unseen market regimes.
